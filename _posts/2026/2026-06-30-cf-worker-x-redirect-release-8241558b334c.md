---
layout: post
date: 2026-06-30 01:10
title: CF Worker X Redirect Version v1.0.7
type: release
cited: GitHub
link: https://github.com/thechelsuk/cf-worker-x-redirect/releases/tag/v1.0.7
release_id: tag:github.com,2008:Repository/1227847157/v1.0.7
release_repo: cf-worker-x-redirect
release_project: cf-worker-x-redirect
release_version: v1.0.7
---

**Full Changelog**: [v1.0.6...v1.0.7](https://github.com/thechelsuk/cf-worker-x-redirect/compare/v1.0.6...v1.0.7)

[Read more about my projects](/projects)
